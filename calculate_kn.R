...
calculateKN <- function(dtx) {
        ...
        dtx <- dtx[dtx$ngram==2, CCN := sum(freq), by = w1]
        dtx <- dtx[dtx$ngram==3, CCN := sum(freq), by = list(w2, w1)]
        ...
        dtx <- dtx[dtx$ngram==1, NWF := .N , by = w1]
        dtx <- dtx[dtx$ngram==2, NWF := .N , by = w1]  # Using data.tables '.N' variable for getting the number of rows per case
        dtx <- dtx[dtx$ngram==3, NWF := .N , by = list(w2, w1)]
        ...
        dtx <- dtx[dtx$ngram==2, NBM := .N , by = w2]
        dtx <- dtx[dtx$ngram==3, NBM := .N , by = w3]
